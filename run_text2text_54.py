# experiment 5: t2t multiplication
...
def build_text2text_model(unique_characters, max_answer_length, learning_rate, dropout_rate):
    ...
    text2text.add(Dropout(dropout_rate))
...
if __name__ == "__main__":
    ...
    learning_rate=1.e-4
    dropout_rate=0.15 #dropout of 0.05 is shown in the report, level is increased to attempt to reduce overfitting
    ...
    np.savez('./results/text2text_54_4l_test_performance.npz', test_sizes=test_sizes, test_accuracies=test_accuracies)
    ...
    for i, txt in enumerate(test_accuracies):
        plt.text(test_sizes[i], test_accuracies[i] - 0.01, f'{txt:.4f}', ha='center', va='top')  

    plt.xlabel('test_size t')
    plt.ylabel('Test Accuracy')
    plt.ylim([0, 1])
    plt.title('text2text_54 model performance across test sizes')
    plt.tight_layout()
    plt.savefig(f'./figures/experiment_5: text2text_54_4layers {epochs} {dropout_rate} {learning_rate}.pdf')
    plt.show()